...
class ESMRDash(pn.viewable.Viewer):
    ...
    parameters = param.ListSelector(
        ...
        objects=['Flow', 'Temperature', 'Dissolved Oxygen',
                        'Electrical Conductivity @ 25 Deg. C'])

    location_place_type = param.ObjectSelector(default='Effluent Monitoring', objects=[
                                               'Effluent Monitoring', 'Receiving Water Monitoring', 'Influent Monitoring', 'Internal Monitoring', 
                                               'Groundwater Monitoring', 'Recycled Water Monitoring', 'Biosolids Monitoring', 'Stormwater Monitoring', 'Non-point Source Monitoring'])
    ...
    def __init__(self, data, **params):
        ...
        self.facility_lat_lon = facility_location_lat_lon.groupby(
            'facility_name').mean().reset_index()
        self.facility_lat_lon = self.facility_lat_lon[['facility_name', 'latitude', 'longitude']]
        self.map = self.facility_lat_lon.hvplot.points(
            ...
            ).opts(frame_width=400, aspect=1,
                active_tools=['wheel_zoom', 'pan', 'tap'], title='Facilities with Lat/Lon information')
    ...
    def plot_facility(self):
        ...
        first_index = index[0]
        print(index)
        return self.plot_parameters(self.data,
    ...
    def __panel__(self):
        return pn.Row(pn.Column(self.map,
