...
class Parameter:
    ...
    def get_variables(self):
        ...
        dfp = self.df[cols].astype(str).replace(
            'nan', '').groupby(cols).count().reset_index()
...
class Facility:
    ...
    def get_locations_of_type(self, location_place_type='Effluent Monitoring'):
        locations = self.get_location_names() #TODO: could be more efficient with direct dataframe ops
        locs = []
        for l in locations:
...
def categorical_types():
    categorical_types = {'region': 'category', 'location': 'category', 'location_place_type': 'category',
                         'parameter': 'category', 'analytical_method_code': 'category', 'analytical_method': 'category',
                         'calculated_method': 'category', 'qualifier': 'category', 'units': 'category',
                         'review_priority_indicator': 'category', 'qa_codes': 'category',
...
def build_facility_location_lat_lon(df):
    ...
    location_lat_lon.loc[location_lat_lon.eval('longitude > -40'), 'longitude'] = -(
        121+3/60.+10.6/3600.)  # another one-off with longitude from description field
...
def get_facilities_with_no_latlon_info(df, facility_location_lat_lon):
    ...
    df_facilites_merged = df_facilities.merge(
        facility_location_lat_lon, how='outer')
    return df_facilites_merged[(df_facilites_merged['latitude'].isnull()) | (df_facilites_merged['longitude'].isnull())]
